    ...
    async getRecommendations(track, artist) {
        try {
            const response = await fetch(
                `/api/recommendations?track=${encodeURIComponent(track)}&artist=${encodeURIComponent(artist)}`
            );
        ...
        container.innerHTML = tracks.slice(0, 5).map(track => `
            <div class="track-card">
                <div class="track-info">
                    ...
                    <button class="save-btn" onclick="player.saveTrack('${track.name}')">💾 Сохранить</button>
                </div>
            </div>
        `).join('');
    }
